...
struct Parser {
	...
	static std::unique_ptr<AST::Type> IdentStrToType() {
		...
		else if(curr_ident == "i32" || curr_ident == "int") { return std::make_unique<AST::Integer32>(); }
	...
	static std::unique_ptr<AST::Expression> ParseIf(bool check_comma = true) {
		...
		while(Lexer::CurrentToken != '}' && Lexer::CurrentToken != Token::Else) {
	...
	static std::unique_ptr<AST::Expression> ParseBlock() {
		...
		if(Lexer::CurrentToken != '{') {
			ExprError("Expected '{' for new block.");
		}
	...
	static std::unique_ptr<AST::Procedure> ParseProcedure() {
		...
		Lexer::isInside = LexerIsInside::AFunction;
		...
		if(Lexer::CurrentToken == ':') { 
			...
			ExprError("Expected ':' to specify procedure type or '{' in procedure.");
		}
		...
		if(Lexer::CurrentToken != '{') { ExprError("Expected '{' in procedure."); }
	...
	static std::unique_ptr<AST::Program> HandleProgram() {
		...
		std::ofstream myfile;
  		myfile.open("llv_main.vale");
  		myfile << program->ToLLVale();
  		myfile.close();
